...
barplot(Q1_df$freq,
        ...
        xlab = "Categories",
        ylab = "Events",
        main = "Categories and Events"
        )

mode <- Q1_df$categories[Q1_df$freq==max(Q1_df$freq)]
mode
...
##Question 5 ###########
Q5_df = data.frame(participant = 1:5, 
                   number_of_toys_used = c(15, 26, 27, 17, 19),
                   speech_fluency = NaN)

speech_fluency.mean = 61.8

plot(Q5_df$number_of_toys_used,
    xlim = range(Q5_df$number_of_toys_used), 
    ylim = c(0, 100),
    xlab = "Number of Toys Used",
    ylab = "Speech Fluency") 
# OR do:
# plot(c(min(Q5_df$number_of_toys_used),max(Q5_df$number_of_toys_used)),
#      c(0, 100),
#      type = "n", #draw a empty plot
#      xlab = "Number of Toys Used",
#      ylab = "Speech Fluency") 

abline(h=speech_fluency.mean, col = "red")

Q5_df$speech_fluency = c(49, 38, 84, 49, 89)

plot(Q5_df$number_of_toys_used, 
     Q5_df$speech_fluency,
     xlim = range(Q5_df$number_of_toys_used), 
     ylim = c(0, 100),
     xlab = "Number of Toys Used",
     ylab = "Speech Fluency")

abline(lm(Q5_df$speech_fluency~Q5_df$number_of_toys_used), col = "blue")
